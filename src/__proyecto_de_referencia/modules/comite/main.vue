...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // remove (id) {
      //   service().delete('/peer_committee/' + id).then(res => {
      //     this.loadData()
      //   })
      // },
      edit (id) {
        service().get('/peer_committee/' + id).then(res => {
      ...
      loadData () {
        service().get('/info/peer_committee').then(res => {
